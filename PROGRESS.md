...
## TASK-B04R iPhone对话区与完整目录定向修复

- 继续时间：2026-08-05 09:02:00（Asia/Shanghai）。开始前确认当前分支及远端功能分支均为`feature/b-level-v2`提交`9efce3a43450100101de2ca76d42f63b9347fee2`，工作区干净；注释保护标签`task-b04a-before-mobile-fix-20260804`剥离后仍指向同一提交。TASK-B05A没有修改本源码仓库。
- 修复前在390×844、360×800、360×500复现并先记录根因：移动端底部快捷入口和场景提示长期占用503px，导致前两种尺寸消息区仅91px和约40px；元素并非fixed/sticky/absolute覆盖，而是底部自动布局行过高。自由文本完整目录又被“已有约束必须为空”条件挡回个性化推荐路径。
- 方案A实现：移动端快捷内容默认收起，首次请求、选择快捷项和键盘关闭时立即收起，只保留不小于44px的“快捷问题”按钮；展开内容保持正常流、限制为`min(28dvh, 210px)`并内部滚动。接近底部才自动跟随的行为保留并抽为可测判定。
- 完整目录意图与个性化推荐意图区分：明确“查看全部班型”“查看所有课程”“有哪些班型”优先生成完整目录并保留但不应用会话约束删卡；个性化措辞继续走既有筛选。类型化课程数据、Prompt、知识事实、费用、日期、grounding和存储规则未修改。
