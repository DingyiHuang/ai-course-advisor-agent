...
“我想学AI”等模糊输入会先澄清身份。信息不足时系统继续采集必要约束，不直接猜测；形成有效约束后推荐真实产品，并可通过“继续咨询该班”继承当前班型追问日期、地点、费用、大纲、设备或物资。课程正文和推荐理由由 LLM 根据本轮结构化事实动态生成，来源由程序依据实际事实编号追加。模型失败、网络异常或接地校验失败会显示脱敏错误并支持重试，不返回固定课程答案冒充成功。
...
- classifier 与 composer 分工的可替换 LLM 适配层
...
确定性代码负责资料域、约束清洗、产品筛选、日期、费用、优惠、前置条件、状态流转、高风险事实校验与来源追加；LLM 只接收已确认事实、程序计算、决策轨迹和短上下文，负责自然语言正文及逐项推荐理由。模型不能自行决定金额、日期或来源章节。
